...
async function callOpenRouterStream(
  exerciseImages: ImagePayload[],
  attemptImages:  ImagePayload[],
  ...
  try {
    ...
    for (const img of exerciseImages) {
      imageContent.push({ type: "image_url", image_url: { url: `data:${img.mime};base64,${img.base64}` } });
    }
    if (!isSolveMode) {
...
router.post(
  "/correct",
  upload.fields([
    { name: "exercise", maxCount: MAX_IMAGES_PER_FIELD },
    { name: "attempt", maxCount: MAX_IMAGES_PER_FIELD },
  ]),
  async (req, res) => {
    try {
      ...
      const exerciseFiles = files?.["exercise"] ?? [];
      const attemptFiles  = files?.["attempt"] ?? [];
      ...
      const mode = (req.body.mode as string) || (attemptFiles.length > 0 ? "correct" : "solve");
      const isSolveMode = mode === "solve" || attemptFiles.length === 0;

      // ── جميع صور التمرين والمحاولة كـ base64 ────────────────────────
      const exerciseImages: ImagePayload[] = exerciseFiles.map(f => ({
        base64: f.buffer.toString("base64"),
        mime: f.mimetype || "image/jpeg",
      }));
      const attemptImages: ImagePayload[] = attemptFiles.map(f => ({
        base64: f.buffer.toString("base64"),
        mime: f.mimetype || "image/jpeg",
      }));

      // الصورة الأولى من كل مجموعة — للتحقق الرياضي السريع بـ mathjs
      const firstExerciseB64  = exerciseImages[0]?.base64 ?? "";
      const firstExerciseMime = exerciseImages[0]?.mime   ?? "image/jpeg";
      const firstAttemptB64   = attemptImages[0]?.base64  ?? "";
      const firstAttemptMime  = attemptImages[0]?.mime    ?? "image/jpeg";
      ...
      try {
        ...
        const mathjsVerification = (!isSolveMode && anyKey)
          ? await Promise.race([
              preAnalyze(firstExerciseB64, firstExerciseMime, firstAttemptB64, firstAttemptMime, anyKey).catch(() => ""),
              sleep(6_000).then(() => ""),
            ])
          : "";

        // ── وصف عدد الصور للذكاء الاصطناعي ─────────────────────────
        const exDesc = exerciseImages.length === 1
          ? "الصورة الأولى: نص التمرين."
          : `الصور الأولى (${exerciseImages.length} صور): نص التمرين موزّع على عدة أوراق — اقرأها بالترتيب وتعامل معها كتمرين واحد متكامل.`;
        const atDesc = attemptImages.length === 1
          ? "الصورة التالية: محاولة الطالب مكتوبة بخط اليد"
          : `الصور التالية (${attemptImages.length} صور): محاولة الطالب مكتوبة بخط اليد على عدة أوراق — اقرأها بالترتيب وتعامل معها كحل واحد متصل`;

        const userMessage = isSolveMode
          ? `الشعبة: **${shoba}**
${notes ? `ملاحظة الطالب: ${notes}` : ""}

${exerciseImages.length === 1 ? "الصورة المرفقة" : `الصور المرفقة (${exerciseImages.length} صور)`}: نص التمرين${exerciseImages.length > 1 ? " موزّع على عدة أوراق — اقرأها بالترتيب" : ""}.
...
${exDesc}
${atDesc} — اقرأها بدقة تامة حتى لو كان الخط غير واضح، واستنتج أي جزء غامض من السياق الرياضي.
...
قيّم محاولة الطالب وفق الهيكل البيداغوجي الإلزامي ومنهاج البكالوريا الجزائرية 2026.`;
        ...
        if (!orSuccess) {
          ...
          const allImages = isSolveMode ? exerciseImages : [...exerciseImages, ...attemptImages];
          const geminiParts: Array<{ inlineData: { data: string; mimeType: string } } | string> = [
            ...allImages.map(img => ({ inlineData: { data: img.base64, mimeType: img.mime } })),
            userMessage,
          ];
